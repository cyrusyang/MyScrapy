...
def mkdir(path):
    if not os.path.exists(path):
        ...
        print
        u"图片存放于:", os.getcwd() + os.sep + path
        return False


def download_pic(img_lists, dir_name):
    print
    "一共有 {num} 张照片".format(num=len(img_lists))
    for image_url in img_lists:
        ...
        try:
            ...
            picture.close
            print
            "下载 {pic_name} 完成!".format(pic_name=file_name)
...
def get_image_url(qid, headers):
    ...
    while True:
        ...
        print
        'size: %d, num : %d' % (size, len(image_urls))


if __name__ == '__main__':
    # question_id = 26037846
    question_id = 22212644
